...
For our application, single threaded performance was needed the most. We switched our server from one being powered by a Xeon E5 server clocked at 2.4Ghz to that of a Xeon E3 server clocked at 3.6Ghz. Though the Dual E5 server node had more overall CPU power, the higher clock rate of the E3 cores allowed us to feel the benefit of the improved single-threaded performance in our application. The DDR3 RAM was also swapped for DDR4 RAM. Though the performance of this change was less notable than many of the other changes, each additional performance we were able to achieve was helpful.
...
Through all of these changes, the speed of our application saw dramatic improvements. It is truly remarkable what can be done by improving the infrastructure of an application. We attempted to eliminate any unnecessary “cost” of computing time that was able to be found.  Why load the same thing multiple times when you can do the expensive computing once, and then simply use the cached result again?
